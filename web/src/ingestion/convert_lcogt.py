...
def convert_obs_file_to_escv(gw_id, healpix_dir, tile_file, tele):
    ...
    for key in table_to_convert.keys():
        if key in ['fieldra', 'r.a.', 'right_ascension', 'RA', 'ra0']:
            table_to_convert.rename_column(key, 'ra')
        if key in ['fielddec', 'declination', 'dec.', 'Dec', 'dec0']:
            table_to_convert.rename_column(key, 'dec')
        if key in ['fieldname', 'object', 'name', 'field', 'Object', 'name']:
            table_to_convert.rename_column(key, 'field_name')
        if key in ['Exp_Time', 'exptime']:
    ...
    for row in table_to_convert:
        ...
        # Sanity for Iair's data
        #   skip non 0425 rows
        #   skip rows with NULL limiting mags, as they can't be used in the analysis
        if not (row['graceid'] == "S190425z" or row['graceid'] == "S190426c") or row['mag_lim'] == "NULL":
            continue
        ...
        elif detect_name in ["SINISTRO"]:
            new_row['source'] = row['graceid'] + "_LCOGT_Prv_Comm"
