...
if is_debug:
    ...
    query_treasure_map_detectors = True
    build_sql_poly = False
    do_tm_pointings_and_plots = False
    ...
    if do_tm_pointings_and_plots:
        ...
        non_tm_tiles = {}
        non_tm_detectors = {}
        select_non_tm_detect = '''
                    SELECT id, Name, ST_AsText(Poly) FROM Detector WHERE TM_id IS NULL;
                '''
        non_tm_detect_result = query_db([select_non_tm_detect])[0]

        select_non_tm_tiles = '''
            SELECT id, Detector_id, RA, _Dec, PositionAngle 
            FROM ObservedTile 
            WHERE HealpixMap_id = %s AND Detector_id = %s;
        '''
        for n_tm_d in non_tm_detect_result:
            n_tm_id = int(n_tm_d[0])
            n_tm_name = n_tm_d[1]
            n_tm_poly = n_tm_d[2]

            if n_tm_name not in non_tm_tiles:
                non_tm_tiles[n_tm_name] = []

            n_tm_vertices = Detector.get_detector_vertices_from_teglon_db(n_tm_poly)
            n_tm_detect = Detector(n_tm_name, n_tm_vertices, detector_id=n_tm_id)
            non_tm_detectors[n_tm_name] = n_tm_detect

            # get tiles
            non_tm_tile_result = query_db([select_non_tm_tiles %
                                           (healpix_map_id, n_tm_detect.id)])[0]
            for ntt in non_tm_tile_result:

                t_id = int(ntt[0])
                t_detector_id = int(ntt[1])
                t_ra = float(ntt[2])
                t_dec = float(ntt[3])
                t_pa = float(ntt[4])

                t = Tile(t_ra, t_dec, n_tm_detect, healpix_map_nside, pa)
                non_tm_tiles[n_tm_name].append(t)
        ...
        if do_full_sky:
            ...
            if only_pixels:
                pix_select = '''
                    SELECT DISTINCT hp.id, hp.Pixel_Index, hp.Prob 
                    FROM HealpixPixel  hp 
                    JOIN ObservedTile_HealpixPixel ot_hp on hp.id = ot_hp.HealpixPixel_id
                    JOIN ObservedTile ot on ot.id = ot_hp.ObservedTile_id 
                    WHERE ot.HealpixMap_id = %s;
                '''
                pix_result = query_db([pix_select % healpix_map_id])[0]
                pix = [Pixel_Element(int(pr[1]), healpix_map_nside, float(pr[2]), pixel_id=int(pr[0]))
                ...
                sql_inner_poly.plot(m, ax, edgecolor='royalblue', linewidth=0.75, facecolor='None')
                sql_outer_poly.plot(m, ax, edgecolor='royalblue', linewidth=0.5, facecolor='None')
                ...
                #                alpha=alph)
                #
                # for detect_name, tile_list in non_tm_tiles.items():
                #     zo = 9999
                #     alph = 1.0
                #     lw = 0.5
                #     for t in tile_list:
                #         t.plot(m, ax, edgecolor=clrs[detect_name], facecolor='None', linestyle='-', linewidth=lw, zorder=zo,
                #                alpha=alph)
                ...
                fig.savefig("GW0425_only_pixels.png", bbox_inches='tight')
                plt.close('all')
            ...
            if mollweide:
                ...
                for detect_name, tile_list in non_tm_tiles.items():
                    ...
                    lw = 0.5
                    for t in tile_list:
                        t.plot(m, ax, edgecolor=clrs[detect_name], facecolor='None', linestyle='-', linewidth=lw, zorder=zo, alpha=alph)
