...
# Load correct light curve models
#Blue KN
# Mej, Vej, Ye
kn_blue_low_velocity_high_mass = "./web/models/kne/33/Metzger_0.325712_0.094828_0.450000.dat"
kn_blue_low_velocity_low_mass = "./web/models/kne/26/Metzger_0.090038_0.094828_0.450000.dat"
kn_blue_high_velocity_high_mass = "./web/models/kne/25/Metzger_0.325712_0.305517_0.450000.dat"
kn_blue_high_velocity_low_mass = "./web/models/kne/36/Metzger_0.090038_0.305517_0.450000.dat"

blues = {
    "lvhm":kn_blue_low_velocity_high_mass,
    "lvlm":kn_blue_low_velocity_low_mass,
    "hvhm":kn_blue_high_velocity_high_mass,
    "hvlm":kn_blue_high_velocity_low_mass
}

#Red KN
# Mej, Vej, Ye
kn_red_low_velocity_high_mass = "./web/models/kne/29/Metzger_0.325712_0.094828_0.100000.dat"
kn_red_low_velocity_low_mass = "./web/models/kne/23/Metzger_0.090038_0.094828_0.100000.dat"
kn_red_high_velocity_high_mass = "./web/models/kne/19/Metzger_0.325712_0.305517_0.100000.dat"
kn_red_high_velocity_low_mass = "./web/models/kne/31/Metzger_0.090038_0.305517_0.100000.dat"

reds = {
    "lvhm":kn_red_low_velocity_high_mass,
    "lvlm":kn_red_low_velocity_low_mass,
    "hvhm":kn_red_high_velocity_high_mass,
    "hvlm":kn_red_high_velocity_low_mass
}
...
for key, mf in blues.items():
    model_table = Table.read(mf, format='ascii.ecsv')
    mask = model_table['sdss_r'] != np.nan
    ...
    H = np.asarray(model_table['ukirt_H'][mask])

    f_r = interp1d(model_time, r, fill_value="extrapolate")
    f_H = interp1d(model_time, H, fill_value="extrapolate")
...
legend_lbl = {
    "lvhm": r"Low $v_{ej}$ / High $M_{ej}$",
    "lvlm": r"Low $v_{ej}$ / Low $M_{ej}$",
    "hvhm": r"High $v_{ej}$ / High $M_{ej}$",
    "hvlm": r"High $v_{ej}$ / Low $M_{ej}$"
}

fig = plt.figure(figsize=(20, 10), dpi=600)
ax_blue = fig.add_subplot(121)
ax_red = fig.add_subplot(122)

for key, light_curves in blue_kn_models.items():
    linesty = plot_styles[key]
    lbl = legend_lbl[key]

    time = light_curves["time"]
    r_lc = light_curves["r"](time)
    H_lc = light_curves["H"](time)

    ax_blue.plot(0,0, color="black", linestyle=linesty, label=lbl, linewidth=3.0)

    ax_blue.plot(time, r_lc, linestyle=linesty, color="blue", linewidth=3.0)
    ax_blue.plot(time, H_lc, linestyle=linesty, color="red", linewidth=3.0)

ax_blue.plot(0,0, color="blue", marker="s", linestyle="None", label="r", linewidth=3.0)
ax_blue.plot(0,0, color="red", marker="s", linestyle="None", label="H", linewidth=3.0)


# ax_blue.legend(loc="lower left", fontsize=14, frameon=False)
ax_blue.set_ylabel("Absolute Mag. (AB)", fontsize=32, labelpad=9.0)
ax_blue.set_xlabel("Days From Merger", fontsize=32)

ax_blue.set_ylim([-18 ,-10])
ax_blue.set_xlim([0, 8])
ax_blue.invert_yaxis()

ax_blue.tick_params(axis='both', which='major', labelsize=24, length=12.0, width=2)



for key, light_curves in red_kn_models.items():
    linesty = plot_styles[key]
    lbl = legend_lbl[key]
    ...
    ax_red.plot(0,0, color="black", linestyle=linesty, label=lbl, linewidth=3.0)

    ax_red.plot(time, r_lc, linestyle=linesty, color="blue", linewidth=3.0)
    ax_red.plot(time, H_lc, linestyle=linesty, color="red", linewidth=3.0)

ax_red.plot(0,0, color="blue", marker="s", linestyle="None", label="r", linewidth=3.0)
ax_red.plot(0,0, color="red", marker="s", linestyle="None", label="H", linewidth=3.0)


ax_red.legend(loc="upper right", fontsize=19, frameon=False)
# ax_red.set_ylabel("Absolute Magnitude (AB)", fontsize=32)
ax_red.set_xlabel("Days From Merger", fontsize=32)

# ax_red.get_shared_y_axes().join(ax_blue, ax_red)
ax_red.set_ylim([-18 ,-10])
ax_red.set_xlim([0, 8])
ax_red.invert_yaxis()

ax_red.grid(linestyle=":")
ax_blue.grid(linestyle=":")

ax_red.tick_params(axis='both', which='major', labelsize=24, length=12.0, width=2)

ax_blue.text(0.2, -17.5, r"$Y_e$ = 0.45", fontsize=32, color="black")
ax_red.text(0.2, -17.5, r"$Y_e$ = 0.10", fontsize=32, color="black")

ax_red2 = ax_red.twinx()
ax_red2.set_ylim([26.5, 18.5])
ax_red2.tick_params(axis='both', which='major', labelsize=24, length=12.0, width=2)
ax_red2.set_ylabel(r"Apparent Mag. ($D_L$=200 Mpc)", fontsize=32, labelpad=9.0)

ax_red.yaxis.set_tick_params(labelleft=False)
# ax_red.yaxis.set_visible(False)

fig.savefig('./web/src/analysis/LightCurve_Comparison.png', bbox_inches='tight')
plt.close('all')
...
#         formatted_healpix_dir = "./web/events/S190425z"
...
#         grb_model_files = {
#             "onaxis": "./web/models/grb/onaxis/9/grb_0.0000_26.8270_0.2511886.dat",
...
#         kn_model_files = {"blue": "./web/models/kne/8/Metzger_0.024890_0.256897_0.450000.dat",
#                           "red": "./web/models/kne/4/Metzger_0.038208_0.143448_0.100000.dat"}
#
#         kn_models = {}
...
#         #
#         # detector_names = ["SWOPE", "THACHER", "NICKEL", "KAIT", "SINISTRO"]
#         # detectors = {}
#         # for d in detector_names:
#         #     detector_result = query_db([detector_select_by_name % d])[0][0]
#         #
...
#         # for ot in ot_result:
#         #
#         #     t = Tile(float(ot[3]), float(ot[4]), detectors["SWOPE"], int(healpix_map_nside), position_angle_deg=position_angle,
#         #              tile_id=int(ot[0]))
#         #
...
#         # print("Loaded %s %s tiles..." % (len(ot_result), "SWOPE"))
#         #
#         #
...
#         #     #          tile_id=int(ot[0]))
#         #     t = Tile(float(ot[3]), float(ot[4]), detectors["NICKEL"], int(healpix_map_nside),
...
#         # print("Loaded %s %s tiles..." % (len(ot_result), "NICKEL"))
#         #
#         #
...
#         #     #          tile_id=int(ot[0]))
#         #     t = Tile(float(ot[3]), float(ot[4]), detectors["THACHER"], int(healpix_map_nside),
#         #              position_angle_deg=position_angle,
#         #              tile_id=int(ot[0]))
#         #     t.field_name = ot[2]
#         #     t.mjd = float(ot[8]) - MERGER_MJD
...
#         # print("Loaded %s %s tiles..." % (len(ot_result), "THACHER"))
#         #
#         #
...
#         #     t = Tile(float(ot[3]), float(ot[4]), detectors["KAIT"], int(healpix_map_nside),
#         #              position_angle_deg=position_angle,
#         #              tile_id=int(ot[0]))
#         #     t.field_name = ot[2]
#         #     t.mjd = float(ot[8]) - MERGER_MJD
...
#         # print("Loaded %s %s tiles..." % (len(ot_result), "KAIT"))
#         #
#         #
...
#         #     #          tile_id=int(ot[0]))
#         #     t = Tile(float(ot[3]), float(ot[4]), detectors["SINISTRO"], int(healpix_map_nside),
#         #              position_angle_deg=position_angle,
#         #              tile_id=int(ot[0]))
#         #     t.field_name = ot[2]
#         #     t.mjd = float(ot[8]) - MERGER_MJD
...
#         # print("Loaded %s %s tiles..." % (len(ot_result), "SINISTRO"))
#
#         # wise_tiles = []
...
#                     ### OLD ###
#
#                     ### Current ### -  temporarily commented out. DC 6/16/2021
...
#
#         if not is_poster_plot and not hide_legend:
#             print("dist mod: %s" % gw0814_dist_mod)
#             axy2 = ax.twinx()
#
...
#         # ax.add_artist(l1)
#         if not is_poster_plot:
#             fig.savefig('GW190814_Lightcurves_onaxis_grb.png', bbox_inches='tight')
#         else:
